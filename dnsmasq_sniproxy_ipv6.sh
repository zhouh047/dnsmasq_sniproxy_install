...
get_ip(){
    local IP=$( ip -6 addr | egrep -o '([a-f0-9:]+:+)+[a-f0-9]+' | egrep -v "^fe80|^fc00|^fd00|::1" | head -n 1 )
    [ -z "${IP}" ] && IP=$( wget -qO- -t1 -T2 ipv6.icanhazip.com )
    [ -z "${IP}" ] && IP=$( wget -qO- -t1 -T2 ifconfig.co/ip )
    echo ${IP}
}

check_ip(){
    local checkip=$1
    if echo $checkip | grep -Eq "^([0-9]{1,3}\.){3}[0-9]{1,3}$"; then
    ...
    elif echo $checkip | grep -Eq "^([a-fA-F0-9:]+:+)+[a-fA-F0-9]+$"; then
        return 0
    else
...
install_dnsmasq(){
    ...
    download /etc/dnsmasq.d/custom_netflix.conf https://raw.githubusercontent.com/zhouh047/dnsmasq_sniproxy_install/refs/heads/dnsmasq_sniproxy_aarch64/dnsmasq-ipv6.conf
    download /tmp/proxy-domains.txt https://raw.githubusercontent.com/zhouh047/dnsmasq_sniproxy_install/refs/heads/dnsmasq_sniproxy_aarch64/proxy-domains-github.txt
...
install_all(){
    ...
    echo ""
    echo -e "${yellow}将您的DNS更改为 $(get_ip) 即可以解析github相关域名了。${plain}"
    echo ""
}
...
only_sniproxy(){
    ...
    echo ""
    echo -e "${yellow}将Netflix的相关域名解析到 $(get_ip) 即可以解析github相关域名了。${plain}"
    echo ""
}
